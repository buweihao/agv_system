...
public sealed class PersistentRouteReservationService : IRouteReservationService
{
    ...
    public PersistentRouteReservationService(AgvDispatcherDbContext db, ITrafficControlService traffic) { _db = db; _traffic = traffic; }
    ...
    public async Task<AgvResult<RouteReservationDto>> CreateReservationAsync(CreateRouteReservationRequest r, CancellationToken ct = default)
    {
        ...
        await Gate.WaitAsync(ct); try { var now = DateTimeOffset.Now; var entity = new RouteReservationEntity { ReservationId = Guid.NewGuid().ToString("N"), TaskId = r.TaskId, VehicleId = r.VehicleId, PlanId = r.PlanId, MapId = r.MapId, MapVersion = r.MapVersion, RollingWindowSize = r.RollingWindowSize, ReservationPolicy = (int)r.ReservationPolicy, State = (int)RouteReservationState.Created, CreatedAt = now, UpdatedAt = now, Segments = r.Segments.OrderBy(x => x.Sequence).Select(x => new RouteReservationSegmentEntity { Sequence = x.Sequence, FromNodeId = x.FromNodeId, ToNodeId = x.ToNodeId, EdgeId = x.EdgeId, Distance = x.Distance, Cost = x.EstimatedSeconds, ResourcesJson = JsonSerializer.Serialize(Resources(x)) }).ToList() }; _db.RouteReservations.Add(entity); await _db.SaveChangesAsync(ct); var dto = Map(entity); if (r.ReservationPolicy == RouteReservationPolicy.LockFullPath) { var locked = await AcquireCoreAsync(entity, 0, int.MaxValue, r.Context, ct); if (!locked.Success || locked.Data?.Acquired != true) return AgvResult<RouteReservationDto>.Fail(FailureCode.InvalidState, locked.Message); dto = Map(entity); } return AgvResult<RouteReservationDto>.Ok(dto); } finally { Gate.Release(); }
    }
    ...
    {
        await Gate.WaitAsync(ct); try { var entity = await LoadAsync(r.ReservationId, ct); if (entity is null) return AgvResult<RouteRollingLockResultDto>.Fail(FailureCode.InvalidRequest, "Route reservation was not found."); return await AcquireCoreAsync(entity, r.CurrentSegmentSequence, r.RollingWindowSize > 0 ? r.RollingWindowSize : entity.RollingWindowSize, r.Context, ct); } finally { Gate.Release(); }
    }
    private async Task<AgvResult<RouteRollingLockResultDto>> AcquireCoreAsync(RouteReservationEntity e, int current, int size, RequestContext context, CancellationToken ct)
    {
        ...
        e.State = (int)(e.Segments.Where(x => !x.IsReleased).All(x => x.IsLocked) ? RouteReservationState.FullyLocked : RouteReservationState.PartiallyLocked); e.UpdatedAt = DateTimeOffset.Now; await _db.SaveChangesAsync(ct); return AgvResult<RouteRollingLockResultDto>.Ok(new RouteRollingLockResultDto { ReservationId = e.ReservationId, State = (RouteReservationState)e.State, Acquired = true, AcquiredWindow = Window(window, e.Segments), Message = "The rolling route window is locked." });
    }
    ...
    {
        await Gate.WaitAsync(ct); try { var e = await LoadAsync(r.ReservationId, ct); if (e is null) return AgvResult<RouteRollingReleaseResultDto>.Fail(FailureCode.InvalidRequest, "Route reservation was not found."); var passed = e.Segments.Where(x => !x.IsReleased && x.Sequence <= r.PassedSegmentSequence).ToArray(); var released = new List<TrafficResourceKey>();
    ...
    public async Task<AgvResult> ReleaseReservationAsync(ReleaseRouteReservationRequest r, CancellationToken ct = default)
    {
        await Gate.WaitAsync(ct); try { var e = await LoadAsync(r.ReservationId, ct); if (e is null) return AgvResult.Fail(FailureCode.InvalidRequest, "Route reservation was not found."); foreach (var s in e.Segments.Where(x => x.IsLocked)) foreach (var id in Ids(s)) { var rr = await _traffic.ReleaseAsync(new TrafficReleaseRequest { Context = r.Context, ReservationId = id, AgvId = e.VehicleId, TaskId = e.TaskId, Resources = Resources(s), Reason = r.Reason }, ct); if (!rr.Success) return rr; }
            ...
            var now = DateTimeOffset.Now; foreach (var s in e.Segments) { s.IsLocked = false; s.IsReleased = true; s.ReleasedAt = now; } e.State = (int)(r.Reason.Contains("cancel", StringComparison.OrdinalIgnoreCase) ? RouteReservationState.Canceled : RouteReservationState.Released); e.ReleasedAt = now; e.UpdatedAt = now; await _db.SaveChangesAsync(ct); return AgvResult.Ok(); } finally { Gate.Release(); }
    }
    public async Task<AgvResult<RouteReservationDto>> GetReservationAsync(GetRouteReservationRequest r, CancellationToken ct = default) { await Gate.WaitAsync(ct); try { var e = await LoadAsync(r.ReservationId, ct); return e is null ? AgvResult<RouteReservationDto>.Fail(FailureCode.InvalidRequest, "Route reservation was not found.") : AgvResult<RouteReservationDto>.Ok(Map(e)); } finally { Gate.Release(); } }
    public async Task<AgvResult<IReadOnlyList<RouteReservationDto>>> GetReservationsByTaskAsync(GetTaskRouteReservationsRequest r, CancellationToken ct = default) { await Gate.WaitAsync(ct); try { var x = await _db.RouteReservations.Include(a => a.Segments).Where(a => a.TaskId == r.TaskId).ToArrayAsync(ct); return AgvResult<IReadOnlyList<RouteReservationDto>>.Ok(x.Select(Map).ToArray()); } finally { Gate.Release(); } }
    public async Task<AgvResult<IReadOnlyList<RouteReservationDto>>> GetReservationsByVehicleAsync(GetVehicleRouteReservationsRequest r, CancellationToken ct = default) { await Gate.WaitAsync(ct); try { var x = await _db.RouteReservations.Include(a => a.Segments).Where(a => a.VehicleId == r.VehicleId).ToArrayAsync(ct); return AgvResult<IReadOnlyList<RouteReservationDto>>.Ok(x.Select(Map).ToArray()); } finally { Gate.Release(); } }

    private Task<RouteReservationEntity?> LoadAsync(string id, CancellationToken ct) => _db.RouteReservations.Include(x => x.Segments).FirstOrDefaultAsync(x => x.ReservationId == id, ct);
