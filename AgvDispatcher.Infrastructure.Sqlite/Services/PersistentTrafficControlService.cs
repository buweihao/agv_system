...
public sealed class PersistentTrafficControlService : ITrafficControlService
{
    ...
    public PersistentTrafficControlService(DbContextOptions<AgvDispatcherDbContext> options) => _options = options;
    ...
    {
        await Gate.WaitAsync(ct); try { await using var db = CreateContext(); await CleanupAsync(db, ct); var rows = await db.TrafficResourceLocks.AsNoTracking().ToArrayAsync(ct); return AgvResult<TrafficSnapshotDto>.Ok(new TrafficSnapshotDto { Version = rows.Sum(x => x.ConcurrencyToken), GeneratedAt = DateTimeOffset.Now, Resources = rows.Select(Map).ToArray() }); } finally { Gate.Release(); }
    }
    ...
    {
        await Gate.WaitAsync(ct); try { await using var db = CreateContext(); await CleanupAsync(db, ct); var row = await FindAsync(db, resource, ct); return AgvResult<TrafficResourceStatusDto>.Ok(row is null ? Free(resource) : Map(row)); } finally { Gate.Release(); }
    }
    ...
    public async Task<AgvResult<TrafficReservationDto>> TryAcquireAsync(TrafficAcquireRequest request, CancellationToken ct = default)
    {
        ...
        {
            await using var db = CreateContext(); await using var tx = await db.Database.BeginTransactionAsync(ct); await CleanupAsync(db, ct); var resources = Distinct(request.Resources);
            foreach (var r in resources) { var held = await FindAsync(db, r, ct); if (held is not null) return AgvResult<TrafficReservationDto>.Fail(FailureCode.InvalidState, $"Resource {r.ResourceType}:{r.ResourceId} is {(TrafficResourceState)held.State}.", (TrafficResourceState)held.State != TrafficResourceState.Disabled); }
            ...
            foreach (var r in resources) db.TrafficResourceLocks.Add(new TrafficResourceLockEntity { ResourceType = (int)r.ResourceType, ResourceId = r.ResourceId, State = (int)State(request.LockMode), OccupiedByAgvId = request.LockMode == TrafficLockMode.Occupy ? request.AgvId : null, ReservedByAgvId = request.LockMode == TrafficLockMode.Occupy ? null : request.AgvId, TaskId = request.TaskId, RouteReservationId = request.Context.CorrelationId, TrafficReservationId = id, LockMode = (int)request.LockMode, Reason = request.Reason, ExpireAt = expires, CreatedAt = now, UpdatedAt = now, ConcurrencyToken = 1 });
            try { await db.SaveChangesAsync(ct); await tx.CommitAsync(ct); } catch (DbUpdateException) { db.ChangeTracker.Clear(); return AgvResult<TrafficReservationDto>.Fail(FailureCode.InvalidState, "A traffic resource was acquired concurrently.", true); }
    ...
    public async Task<AgvResult> ReleaseAsync(TrafficReleaseRequest request, CancellationToken ct = default)
    {
        ...
        await Gate.WaitAsync(ct); try { await using var db = CreateContext(); await CleanupAsync(db, ct); var rows = request.Resources.Count == 0 ? await db.TrafficResourceLocks.ToListAsync(ct) : await RowsAsync(db, request.Resources, ct); var matches = rows.Where(x => Owner(x, request)).ToArray(); db.RemoveRange(matches); await db.SaveChangesAsync(ct); return AgvResult.Ok($"Released {matches.Length} traffic resource(s)."); } finally { Gate.Release(); }
    }
    public async Task<AgvResult> UpdateAgvOccupancyAsync(AgvOccupancyUpdateRequest request, CancellationToken ct = default)
    {
        ...
        await Gate.WaitAsync(ct); try { await using var db = CreateContext(); await CleanupAsync(db, ct); foreach (var r in reported) { var held = await FindAsync(db, r, ct); if (held is not null && ((TrafficResourceState)held.State is TrafficResourceState.Blocked or TrafficResourceState.Disabled || (!string.Equals(held.OccupiedByAgvId, request.AgvId, StringComparison.OrdinalIgnoreCase) && !string.Equals(held.ReservedByAgvId, request.AgvId, StringComparison.OrdinalIgnoreCase)))) return AgvResult.Fail(FailureCode.InvalidState, $"Resource {r.ResourceType}:{r.ResourceId} is owned by another AGV."); }
            var keys = reported.Select(Key).ToHashSet(); var old = await db.TrafficResourceLocks.Where(x => x.State == (int)TrafficResourceState.Occupied && x.OccupiedByAgvId == request.AgvId).ToListAsync(ct); db.RemoveRange(old.Where(x => !keys.Contains($"{x.ResourceType}:{x.ResourceId}")));
            foreach (var r in reported) { var row = await FindAsync(db, r, ct); if (row is null) { row = new TrafficResourceLockEntity { ResourceType = (int)r.ResourceType, ResourceId = r.ResourceId, CreatedAt = DateTimeOffset.Now }; db.Add(row); } row.State = (int)TrafficResourceState.Occupied; row.OccupiedByAgvId = request.AgvId; row.ReservedByAgvId = null; row.TaskId = request.TaskId; row.TrafficReservationId = null; row.Reason = "AGV occupancy report"; row.UpdatedAt = request.ReportTime; row.ConcurrencyToken++; }
            await db.SaveChangesAsync(ct); return AgvResult.Ok(); } finally { Gate.Release(); }
    }
    ...
    {
        await Gate.WaitAsync(ct); try { await using var db = CreateContext(); var rows = await RowsAsync(db, request.Resources, ct); var blocked = rows.Where(x => x.State == (int)TrafficResourceState.Blocked).ToArray(); db.RemoveRange(blocked); await db.SaveChangesAsync(ct); return AgvResult.Ok($"Unblocked {blocked.Length} traffic resource(s)."); } finally { Gate.Release(); }
    }

    private AgvDispatcherDbContext CreateContext() => new(_options);
    private static async Task CleanupAsync(AgvDispatcherDbContext db, CancellationToken ct) { var now = DateTimeOffset.Now; var expirable = await db.TrafficResourceLocks.Where(x => x.ExpireAt != null).ToListAsync(ct); var expired = expirable.Where(x => x.ExpireAt <= now).ToArray(); if (expired.Length > 0) { db.RemoveRange(expired); await db.SaveChangesAsync(ct); } }
    private static Task<TrafficResourceLockEntity?> FindAsync(AgvDispatcherDbContext db, TrafficResourceKey r, CancellationToken ct) => db.TrafficResourceLocks.FirstOrDefaultAsync(x => x.ResourceType == (int)r.ResourceType && x.ResourceId == r.ResourceId, ct);
    private static async Task<List<TrafficResourceLockEntity>> RowsAsync(AgvDispatcherDbContext db, IEnumerable<TrafficResourceKey> resources, CancellationToken ct) { var keys = Distinct(resources).Select(Key).ToHashSet(); return (await db.TrafficResourceLocks.ToListAsync(ct)).Where(x => keys.Contains($"{x.ResourceType}:{x.ResourceId}")).ToList(); }
